...
#[tauri::command]
async fn open_adopted_pet_window(
    app: tauri::AppHandle,
    pet_id: String,
    asset_id: String,
) -> Result<(), String> {
    ...
    let url = format!("index.html?surface=pet-window&petId={pet_id}&assetId={asset_id}&windowIndex=1");
...
#[cfg(target_os = "windows")]
#[tauri::command]
fn focus_window(hwnd: i64) -> Result<bool, String> {
    ...
    unsafe {
        ...
        // ponytail: AttachThreadInput dance because Windows blocks a bare
        // SetForegroundWindow from a process that isn't the active one.
...
/// launched window so the caller can bind it (None if it didn't surface in time
/// — the terminal still opened, it just isn't bound).
#[cfg(target_os = "windows")]
#[tauri::command]
fn start_session(cwd: String) -> Result<Option<ForeignWindow>, String> {
...
#[tauri::command]
fn focus_window(_hwnd: i64) -> Result<bool, String> {
    Err("Window focus is only implemented on Windows".to_string())
}

#[cfg(not(target_os = "windows"))]
#[tauri::command]
fn start_session(_cwd: String) -> Result<Option<ForeignWindow>, String> {
