...
- `lib/feature-points.ts` — the feature section's points and their demo clips; shared by that section and the layout's JSON-LD `featureList`
- `components/` — `Intro.tsx` (largest file), `IntroScenes.tsx`, `FeatureTabs.tsx`, `DemoVideo.tsx`, `DownloadButton.tsx`, `LanguageSwitcher.tsx`, `GithubLink.tsx`
...
- **The feature section deliberately skips `data-reveal`.** Every other section starts at `opacity: 0` until an `IntersectionObserver` fires; this one holds the page's indexable prose — a lede plus every tab panel, all but one of them hidden — and has to be visible in the HTML as served.
- **The feature tabs render every panel and hide all but one.** `FeatureTabs` hides the inactive panels with the `hidden` attribute rather than leaving them unrendered, so every title and body ships in the served HTML — text behind a tab is indexed, text that was never rendered is not. The clips get the opposite treatment: only the active panel mounts a `<video>`, because one per point is the eager media load `DemoVideo` exists to prevent. The same points are restated as `featureList` in the layout's JSON-LD, and that list has to keep matching what the panels say. Each tab carries an icon from `TAB_ICONS`, and the strip scrolls inside its own pill rather than growing past the 1080px container — so another point costs a tab the visitor has to scroll to, not a broken layout. Every point plays footage of its own behaviour and each `alt` describes that clip; a point that has to borrow a clip words its `alt` after the picture, not the point.
