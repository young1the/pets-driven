...
- **`pdd worktree` keeps no registry of its own** (`pets-driven-cli/src/worktree.rs`). Git already records which folders are worktrees of which repository, so the commands shell out to `git worktree list` and bind an ordinary pet to the new folder — no `state.v1.json` field, no migration, and a worktree made with plain `git` still lists. Every git call goes through the `Git` seam so the commands test against a scripted repository.
