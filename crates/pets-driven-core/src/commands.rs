...
const PET_VOICE_PITCH_MIN: f64 = 0.9;
const PET_VOICE_PITCH_MAX: f64 = 1.9;

/// Keep Pet Birth deterministic while spreading random ids over a stable pitch range.
fn default_voice_pitch(pet_id: &str) -> f64 {
    let mut hash = 0x811c9dc5_u32;
    for code_unit in pet_id.encode_utf16() {
        hash ^= u32::from(code_unit);
        hash = hash.wrapping_mul(0x01000193);
    }
    let pitch = 1.1 + 0.6 * (f64::from(hash) / f64::from(u32::MAX));
...
pub(crate) fn apply_hatch(
    ...
    push_array(
        ...
        serde_json::json!({
            ...
            "voice": {
                "pitch": default_voice_pitch(&ids.pet_id),
                "muted": false
            }
        }),
    );
...
pub(crate) fn apply_pet_update(
    ...
    if let Some(pitch) = patch.voice_pitch {
        if !pitch.is_finite() || !(PET_VOICE_PITCH_MIN..=PET_VOICE_PITCH_MAX).contains(&pitch) {
            return Err(CoreError::field(format!(
                "voicePitch must be between {PET_VOICE_PITCH_MIN} and {PET_VOICE_PITCH_MAX}"
            )));
        }
    }
    ...
    if personality_patch.is_some()
        || patch.asset_id.is_some()
        || patch.voice_pitch.is_some()
        || patch.voice_muted.is_some()
    {
        if let Some(profiles) = next.get_mut("petProfiles").and_then(|value| value.as_array_mut()) {
            for profile in profiles.iter_mut() {
                ...
                if patch.voice_pitch.is_some() || patch.voice_muted.is_some() {
                    let voice = object.entry("voice").or_insert_with(|| serde_json::json!({}));
                    if !voice.is_object() {
                        *voice = serde_json::json!({});
                    }
                    let voice = voice
                        .as_object_mut()
                        .expect("voice was normalized to an object");
                    if let Some(pitch) = patch.voice_pitch {
                        voice.insert("pitch".to_string(), serde_json::json!(pitch));
                    }
                    if let Some(muted) = patch.voice_muted {
                        voice.insert("muted".to_string(), serde_json::json!(muted));
                    }
                }
            }
        }
...
mod tests {
    ...
    fn apply_hatch_appends_pet_profile_and_directory() {
        ...
        assert_eq!(profile["voice"]["pitch"], default_voice_pitch("pet-1"));
        assert_eq!(profile["voice"]["muted"], false);
    ...
    #[test]
    fn apply_pet_update_persists_voice_settings_on_the_profile() {
        let next = apply_pet_update(
            &hatched_state(),
            &PetId::new("pet-1"),
            &PetPatch {
                voice_pitch: Some(1.62),
                voice_muted: Some(true),
                ..blank_patch()
            },
            &sample_working_directory_ids(),
            2000,
        )
        .expect("voice update should succeed");

        assert_eq!(next["petProfiles"][0]["voice"]["pitch"], 1.62);
        assert_eq!(next["petProfiles"][0]["voice"]["muted"], true);
    }

    #[test]
    fn apply_pet_update_rejects_an_out_of_range_voice_pitch_atomically() {
        ...
        let error = apply_pet_update(
            ...
            &PetPatch {
                ...
                voice_pitch: Some(2.5),
                ..blank_patch()
            },
            &sample_working_directory_ids(),
            2000,
        )
        .expect_err("an unsupported voice pitch must be rejected");

        assert!(error.to_string().contains("voicePitch"));
