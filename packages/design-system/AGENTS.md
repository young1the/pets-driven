...
- **`Tabs` scrolls inside its own pill; it never grows past its container.** A tab is `white-space: nowrap` and, like any flex item, has `min-width: auto`, so it cannot shrink below its label — `.pd-tabs` therefore caps at `max-width: 100%` and takes `overflow-x: auto`. Only `.pd-tabs--block` (the `fullWidth` prop) stretches its tabs with `flex: 1`; the default pill sizes them by their labels. The gallery's "Tabs — overflowing" cell is where to check this.
