...
- **A tool event is a pulse, not a task start.** `tool.used` refreshes an already-working pet without re-speaking, re-claiming, or resetting `since` — tools fire far faster than the 5s agent-event claim, so treating them as task starts pins the pet under that claim for the whole session and no working pose ever renders. It only records `AgentToolActivity`, which the working behavior reads to pick a pose.
- **The working state is an ordinary behavior, not a special sprite case.** A working pet holds a personality-picked pose claim (`src/pets/personalities/working-styles.ts`) that renders through the same choreography and activity-label machinery as autonomous poses. A new working beat means a new reason in `working-styles.ts`, a choreography in `pose-choreography.ts`, an activity in `core/pet-activity.ts`, and a label in `pet-status-presentation.ts` plus the i18n bundles — not a branch in the animation mapper.
